...
function RegisterModal({ onClose, onRegisterSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      const data = await register({ name: form.name, email: form.email, password: form.password, phone: form.phone || undefined })
        ...
        <div className="login-modal-card" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="register-modal-success">
            <div className="pokeball-icon">
              ...
              <div className="pokeball-mid"><div className="pokeball-btn" /></div>
            ...
            <button className="submit-btn" onClick={() => { onRegisterSuccess(); onClose() }}>
              시작하기
            </button>
          </div>
        </div>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="login-modal-card register-modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <form className="login-modal-form register-modal-form" onSubmit={handleSubmit} noValidate>

          <div className="field-group">
            <label htmlFor="rm-name">트레이너 이름 <span className="required">*</span></label>
            <input id="rm-name" name="name" type="text" placeholder="이름을 입력하세요"
          ...
          <div className="field-group">
            <label htmlFor="rm-email">이메일 <span className="required">*</span></label>
            <input id="rm-email" name="email" type="email" placeholder="example@pokemon.com"
          ...
          <div className="field-group">
            <label htmlFor="rm-password">비밀번호 <span className="required">*</span></label>
            <input id="rm-password" name="password" type="password" placeholder="6자 이상 입력하세요"
          ...
          <div className="field-group">
            <label htmlFor="rm-confirm">비밀번호 확인 <span className="required">*</span></label>
            <input id="rm-confirm" name="confirmPassword" type="password" placeholder="비밀번호를 한 번 더 입력하세요"
              value={form.confirmPassword} onChange={handleChange}
              className={errors.confirmPassword ? 'input-error' : ''} autoComplete="new-password" />
            {errors.confirmPassword && <p className="error-msg">{errors.confirmPassword}</p>}
          </div>

          <div className="field-group">
            <label htmlFor="rm-phone">전화번호 <span className="optional">(선택)</span></label>
            <input id="rm-phone" name="phone" type="tel" placeholder="010-0000-0000"
              value={form.phone} onChange={handleChange} />
          </div>

          <div className={`terms-box ${errors.agreements ? 'terms-box-error' : ''}`}>
            <label className="agree-all-row">
              <input type="checkbox" checked={allChecked} onChange={handleAgreeAll} className="agree-checkbox" />
              ...
              <div key={term.id} className="term-item">
                <div className="term-row">
                  <label className="term-check-label">
                    <input type="checkbox" checked={agreements[term.id]}
                      onChange={() => handleAgreement(term.id)} className="agree-checkbox" />
                  ...
                  <button type="button" className="term-toggle"
                    onClick={() => toggleExpand(term.id)} aria-expanded={!!expanded[term.id]}>
                    {expanded[term.id] ? '닫기 ▲' : '보기 ▼'}
                  </button>
                </div>
                {expanded[term.id] && <div className="term-content">{term.content}</div>}
              </div>
            ))}
          ...
          <button type="submit" className="submit-btn" disabled={loading}>
            {loading ? (
              <span className="btn-loading"><span className="spinner" />등록 중...</span>
            ) : '트레이너 등록하기'}
          </button>

          <p className="login-link">
            이미 계정이 있으신가요?{' '}
            <a href="#" onClick={(e) => { e.preventDefault(); onClose() }}>로그인</a>
          </p>
